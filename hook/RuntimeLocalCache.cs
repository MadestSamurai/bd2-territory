...
namespace BD2Territory.Runtime
{
 internal sealed partial class RuntimeEngine
 {
  ...
  {return localGround.Get(new RouteSampleKey(p),DateTime.UtcNow.Ticks,()=>{Vector3 ground;return GroundPoint(Vector(p),out ground,true,false)?(RoutePoint?)Point(ground):null;});}
  ...
  private void RefreshLocalCacheContext()
  {
   ...
   string shape=body==null?"default":body.radius.ToString("R")+"/"+body.height.ToString("R")+"/"+body.center.ToString("F4")+"/"+body.transform.lossyScale.ToString("F4");
