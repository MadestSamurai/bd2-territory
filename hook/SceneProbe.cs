  ...
  internal static void Capture(string reason)
  {
   try
   {
    ...
    foreach(var assembly in AppDomain.CurrentDomain.GetAssemblies().Where(a=>(a.GetName().Name??"").StartsWith("BD2Territory.Runtime",StringComparison.Ordinal)))
    {
     ...
     foreach(var name in new[]{"targetNode","walkTarget","walkDestination","finalDestination","blockingCollider","navigation","gatheringReposition","gathering","gatheringStand","gatheringStands","vehiclePending","ownsVehicle","escapeStart","escapeDirection","localMoving","localIndex","localSearch","localBudget","localBlocked","localRoute","localTarget","localPlanAt","localProgressAt","npcFootprints"})
     {var f=active.GetType().GetField(name,BindingFlags.Instance|BindingFlags.NonPublic);var value=f?.GetValue(active);text.AppendLine(" engine "+name+"="+value);if(name=="npcFootprints"&&value is System.Collections.IEnumerable actors)foreach(var actor in actors)text.AppendLine(" npc="+actor);if(value is Component component)text.AppendLine(" selectedId="+component.GetInstanceID()+" pos="+Pos(component.transform.position));if(value is System.Collections.Generic.IEnumerable<Vector3> points)text.AppendLine(" points="+string.Join(";",points.Select(Pos).ToArray()));}
    }
    ...
    foreach(var c in Physics.OverlapSphere(player.transform.position,4,~0,QueryTriggerInteraction.Ignore).Where(c=>!c.transform.IsChildOf(player.transform)))text.AppendLine("nearObstacle="+c.name+" type="+c.GetType().Name+" enabled="+c.enabled+" layer="+c.gameObject.layer+" ignoredPlayerLayer="+Physics.GetIgnoreLayerCollision(player.gameObject.layer,c.gameObject.layer)+" ignoredPlayerPair="+Physics.GetIgnoreCollision(player.GetComponent<CharacterController>(),c)+" rotation="+Pos(c.transform.eulerAngles)+" center="+Pos(c.bounds.center)+" size="+Pos(c.bounds.size)+(c is BoxCollider box?" localCenter="+Pos(box.center)+" localSize="+Pos(box.size):""));
