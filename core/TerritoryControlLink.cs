...
public sealed class TerritoryControlLink:IDisposable
{
 ...
 public void Configure(TerritorySettings s)
 {
  ...
  lock(sync){command.Cooking=s.Cooking;command.CookingBatch=s.CookingBatch;command.RecipeId=s.RecipeId;command.Logging=s.Logging;command.Mining=s.Mining;command.Farming=s.Farming;command.DashRecovery=s.DashRecovery;command.UseVehicle=s.UseVehicle;command.IntervalMs=s.IntervalMs;command.PlantingBudget=s.PlantingBudget;TerritoryJson.Write(Path.Combine(root,"settings.json"),s);Write();}
 }
