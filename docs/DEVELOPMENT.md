...
Water is a hard surface constraint, independent of collider prediction or the trial flag. `RuntimeSurface` uses `LifeChunkController.WorldPositionToWorldCell` and the current chunk cell type; unknown/None cells fail closed. Water requires an active native `LifePlaceableObject_ColliderGate` on a placed, non-preview water object, inside the oriented gate inset by the actual body radius. It does not change collision filters. The bridge gate may itself ignore the broad ground collider; that floor can support the authored crossing only inside the verified bridge corridor. `SurfaceSegments` splits lines at terrain-cell boundaries as well as short intervals, so sampling cannot skip a water corner. Execution checks the live bridge and terrain again; temporary physical-obstacle trials cannot relax this guard.
