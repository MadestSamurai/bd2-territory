...
namespace SharpMonoInjector
{
    public class Injector : IDisposable
    {
        ...
        public void Invoke(IntPtr assembly, string @namespace, string className, string methodName)
        {
            if (assembly == IntPtr.Zero)
                throw new ArgumentException($"{nameof(assembly)} cannot be zero", nameof(assembly));
        ...
        private void RuntimeInvoke(IntPtr method)
        {
            ...
            if (exc != IntPtr.Zero) {
                ...
                IntPtr messageObject = Is64Bit
                    ? (IntPtr)_memory.ReadLong(exc + 0x20)
                    : (IntPtr)_memory.ReadInt(exc + 0x10);
                string message = messageObject == IntPtr.Zero
                    ? "managed exception message unavailable"
                    : ReadMonoString(messageObject);
