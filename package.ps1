...
foreach($flavor in @('Portable','Lite')){
    ...
    if($identity.runtime -ne 'BD2Territory.Runtime14' -or $identity.compatibility -ne 'local-interface-adaptation' -or $identity.defaultIntervalMs -ne 500){throw 'Embedded identity differs from release'}
